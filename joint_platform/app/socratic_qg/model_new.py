...
class QG_Model(object):
    def __init__(self) -> None:
        ...
        self.model_sbert = SentenceTransformer('/sbert_model/', device=device)
    ...
    def generate_all_labels(self, context):
        ...
        for label, repetition_penalty, temperature, sample_size in zip(self.labels, self.repetition_penalties, self.temperatures, self.sample_sizes):       
            for generation in self.generate(label = label, context=context, sample_size=sample_size, repetition_penalty=repetition_penalty, temperature=temperature):
                output[label].append(generation)
            print("-")
...
if __name__ == "__main__":
    qg = QG_Model()
    qg.generate_all_labels("I know you won't believe me, but the highest form of human excellence is to question oneself and others.")
