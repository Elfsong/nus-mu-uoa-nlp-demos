...
class ClusteringAlgo:
    ...
    def iter_on_matrix(self, ):
        if self.distance == "precomputed":
            ...
            for idx in range(0, matrix.shape[0], self.batch_size):
                if idx % 10000 == 0:
                    logging.info(idx)
                vectors = matrix[idx:min(
                    idx + self.batch_size, matrix.shape[0])]
                yield idx, vectors

    def brute_nn(self, data, tweets):
        nn = NearestNeighbors(
            n_neighbors=1, algorithm='brute', metric=self.distance)
        if self.distance == "precomputed":
    ...
    def incremental_clustering(self, ):
        ...
        for idx, tweets in self.iter_on_matrix():
            ...
            if idx == 0:
                ...
                new_labels = np.arange(
                    self.thread_id, self.thread_id + len(distant_neighbors))
                threads[i:j][~under_t] = new_labels

                if new_labels.size != 0:
                    self.thread_id = max(new_labels) + 1
            if issparse(self.M):
                # much faster than T[i:j] = tweets
        ...
        total_threads_with_zeros_vectors = np.zeros(
            self.M.shape[0], dtype="int")
    ...
    def incremental_clustering(self, ):
        ...
        for i, tweets, mask, T, window_mask in self.iter_on_matrix(matrix):
            ...
            if i == 0:
                ...
                under_t = np.array(distances) < self.t
                # points that have a close neighbor in the window get the label of that neighbor
                threads[i:j][under_t] = threads[max(
                    0, i - self.w):i][window_mask][neighbors[under_t]]
                ...
                new_labels = range(
                    self.thread_id, self.thread_id + clustered.sum())
